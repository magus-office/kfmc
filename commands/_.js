...
function successMessage(res, amount){
  ...
  Api.sendMessage({ chat_id: "@akkjshhsbhjwjbv", text: name + " \n\n📜 Successful!\n\n " + other_name + " (Amount) " + prefix + amount + " ₹"  } );
}
...
function transferByUser(res, anotherRes, amount){
  if(!res.have(amount)){
     Bot.sendMessage("⚠ Infucient Balance!\nYou have only: " +
...
function transferByMaster(res, anotherRes, amount){
  ...
  if(!mp.have(amount)){
    Bot.sendMessage("Daily Limit Reached! ⚠.\n💰Curreent: " +
    mp.value() + " \n⚠ Try again after 24h.", {is_reply: true});
    return false;
  }
